...
export default function Sale() {
  ...
  const deleteProduct = (id) => {
    axios
      .delete(`${endpoint}/Sale/delete/${id}`)
      .then((response) => {
  ...
  const HandelSubmit = async (e) => {
    try {
      if (isedit) {
        ...
        let updatedEndPoint = `${endpoint}/Sale/update/${saleObj._id}`;
        await axios
        ...
        await axios
          .post(`${endpoint}/AddSale`, saleObj, headers)
          .then((response) => {
  ...
  const handelChange = (e) => {
    setsaleObj({ ...saleObj, [e.target.name]: e.target.value });
  };
  const tog_standard = () => {
    setisopen(!isopen);
  };
    ...
    <div className="d-flex">
      ...
      <div className="dash">
        ...
        <Col className="col-11" style={{ marginLeft: "30px" }}>
          ...
          <Card>
            <CardBody>
              <CardTitle className="h4">Sale table </CardTitle>
